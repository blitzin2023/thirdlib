...
struct __db_env {
	...
	unsigned int		mp_ncache;	/* Initial number of cache regions */
...
#define DB_ENV_NOFLUSH		0x00080000 /* DB_NOFLUSH set */
	...
	int  (*repmgr_set_incoming_queue_max)
		__P((DB_ENV *, u_int32_t, u_int32_t));
	int  (*repmgr_site)
		__P((DB_ENV *, const char *, unsigned int, DB_SITE**, u_int32_t));
	int  (*repmgr_site_by_eid) __P((DB_ENV *, int, DB_SITE**));
	int  (*repmgr_site_list) __P((DB_ENV *, unsigned int *, DB_REPMGR_SITE **));
...
struct __db_site {
	...
	int (*get_address) __P((DB_SITE *, const char **, unsigned int *));
...
int db_env_set_func_yield __P((int (*)(unsigned long, unsigned long)));
